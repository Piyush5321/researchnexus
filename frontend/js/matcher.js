...
class PaperMatcherController {
  ...
  renderResults(data) {
    ...
    const kernelsHtml = (doc.keyMathematicalKernels || [])
      .map(k => `<span class="badge" style="background: rgba(0, 240, 255, 0.1); border-color: rgba(0, 240, 255, 0.3); color: var(--neon-cyan);"><i class="fa-solid fa-microchip" style="font-size: 10px;"></i> ${k}</span>`)
      .join(' ');
        ...
        <div class="match-item glass-card" style="padding: 16px; margin-bottom: 14px; border-radius: 12px; border: 1px solid rgba(255, 255, 255, 0.08);">
          <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 10px;">
            <div>
              ...
              <span class="badge" style="background: rgba(138, 43, 226, 0.15); border-color: rgba(138, 43, 226, 0.4); color: var(--neon-violet-light);">
                <i class="fa-solid fa-code-compare"></i> ${item.genreOverlap}
              </span>
            </div>
            <div style="text-align: right;">
              <span style="font-family: var(--font-mono); font-size: 18px; font-weight: 700; color: var(--neon-cyan);">${matchPct}%</span>
          ...
          <p style="color: var(--text-muted); font-size: 12px; margin-bottom: 10px;">
            <i class="fa-solid fa-user-astronaut" style="color: var(--neon-cyan);"></i> Lead: <strong style="color: #fff;">${item.author}</strong>
          </p>

          <div style="background: rgba(0, 0, 0, 0.3); border-radius: 8px; padding: 10px; margin-bottom: 12px;">
            <div style="font-size: 11px; color: var(--alert-amber); font-weight: 600; margin-bottom: 4px;">
              <i class="fa-solid fa-lightbulb"></i> Recommended Cross-Department Collaboration:
            </div>
            <p style="font-size: 12px; color: var(--text-muted); margin: 0; line-height: 1.4;">${item.recommendedCollaboration}</p>
          </div>

          <div style="display: flex; justify-content: flex-end; gap: 8px;">
            <button class="btn btn-ghost btn-sm" onclick="window.FocusGraphNode('${item.id}')">
    ...
    this.resultsContainer.innerHTML = `
      <div class="glass-panel" style="padding: 20px; border-radius: 14px; margin-bottom: 20px; border-color: rgba(0, 240, 255, 0.2);">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 14px;">
          <div>
            <div style="font-size: 11px; text-transform: uppercase; letter-spacing: 1px; color: var(--text-dim);">AI Classification Result</div>
            <h3 style="color: #fff; font-size: 18px; margin: 4px 0 0 0;">${doc.detectedGenre}</h3>
          </div>
          <span class="badge" style="background: rgba(0, 250, 100, 0.15); border-color: rgba(0, 250, 100, 0.4); color: var(--neon-green); font-size: 12px;">
            <i class="fa-solid fa-circle-check"></i> ${Math.round(doc.genreConfidence * 100)}% Confidence
          </span>
        </div>

        <div style="margin-bottom: 10px;">
          <div style="font-size: 11px; color: var(--text-dim); margin-bottom: 6px;">Identified Mathematical & Algorithmic Kernels:</div>
      ...
      <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 14px;">
        <h4 style="color: #fff; font-size: 14px; text-transform: uppercase; letter-spacing: 0.5px; margin: 0;">
          <i class="fa-solid fa-network-wired" style="color: var(--neon-cyan);"></i> Cross-Disciplinary Matches (${matches.length})
        </h4>
        <span style="font-size: 11px; color: var(--text-dim);">AlloyDB pgvector Cosine Rank</span>
      </div>
...
window.OpenCollaborationBrief = function(id, title, author) {
  const matcher = window.AppMatcher;
  if (matcher) matcher.closeModal();
  if (window.openConnectModal) {
    window.openConnectModal('ALERT-NAV-89');
  }
};
